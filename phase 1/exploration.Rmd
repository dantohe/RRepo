...
sfo_feb_flights %>% summarise(mean.arrival.delay = mean(arr_delay) , median.arrival.delay = median(arr_delay), sd.arrival.delay= sd(arr_delay) )
boxplot(sfo_feb_flights$arr_delay, horizontal = T)
```
...
sfo_feb_flights %>% group_by(carrier) %>% summarise(median.arr.delay = median(arr_delay), iqr.arr.delay = IQR(arr_delay))
```
**Answer: Delta and UA**  
...
data(nycflights)
nycflights <- nycflights %>%
  mutate(avg_speed = distance /(air_time/60)) %>% select(avg_speed, tailnum)
nycflights[nycflights$avg_speed==max(nycflights$avg_speed),]
data(nycflights)
```
**answer N666DN**
